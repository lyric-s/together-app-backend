...
@router.get("/ai-reports", response_model=list[AIReportPublic])
def get_all_ai_reports(
    *,
    ...
    """
    Retrieve all AI-generated moderation reports.

    Returns a paginated list of all reports automatically generated by the AI
    moderation system, ordered by creation date (newest first). This allows
    administrators to review content flagged by the system.

    ### Authorization Required:
    - **Admin authentication**: Requires a valid admin access token.
    - **Admin mode**: Token must include the "mode": "admin" claim.
    ...
    Returns:
        list[AIReportPublic]: A list of AI reports with classification and confidence scores.
    """
    reports = ai_report_service.get_ai_reports(session, offset=offset, limit=limit)
    return [AIReportPublic.model_validate(r) for r in reports]


@router.patch("/ai-reports/{report_id}", response_model=AIReportPublic)
def update_ai_report_state(
    *,
    ...
    ### State Transitions:
    - PENDING → APPROVED: Accept the AI prediction as valid.
    - PENDING → REJECTED: Dismiss the AI prediction as a false positive.

    Args:
        report_id: The unique identifier of the AI report to update.
        session: Database session.
        current_admin: Authenticated admin.
        report_update: Update data containing the new state (APPROVED/REJECTED).

    Returns:
        AIReportPublic: The updated AI report instance.
    """
    report = ai_report_service.update_ai_report_state(session, report_id, report_update)
    session.commit()
    session.refresh(report)
    return AIReportPublic.model_validate(report)
