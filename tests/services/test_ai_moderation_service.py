...
from app.models.enums import AIContentCategory, ReportTarget
...
@pytest.mark.asyncio
async def test_moderate_content_spam(
    session: Session, ai_service: AIModerationService, volunteer_user
):
    ...
    reported_user_id = volunteer_user.id_user

    await ai_service.moderate_content(
        session, ReportTarget.PROFILE, target_id, reported_user_id, text
    )
...
@pytest.mark.asyncio
async def test_moderate_content_toxic(
    session: Session, ai_service: AIModerationService, volunteer_user
):
...
async def test_skip_if_human_report_exists(
    session: Session, ai_service: AIModerationService, volunteer_user, reporter_user
):
    ...
    human_report = Report(
        ...
        id_user_reporter=reporter_user.id_user,
    )
    session.add(human_report)
    session.commit()

    await ai_service.moderate_content(
        session, ReportTarget.PROFILE, target_id, reported_user_id, "spam text"
    )
...
@pytest.mark.asyncio
async def test_batch_moderation_logic(
    session: Session, ai_service: AIModerationService, volunteer_user
):
    """Test the batch moderation candidate selection."""
