...
def test_update_ai_report_state(session: Session, sample_ai_report: AIReport):
    ...
    updated = ai_report_service.update_ai_report_state(
        session, sample_ai_report.id_report, update_data
    )
...
def test_update_ai_report_state_not_found(session: Session):
    ...
    with pytest.raises(NotFoundError):
        ai_report_service.update_ai_report_state(
            session, 9999, AIReportUpdate(state=ProcessingStatus.REJECTED)
        )
