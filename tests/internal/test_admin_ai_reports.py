...
def test_get_all_ai_reports(
    client: TestClient, admin_token: str, sample_ai_report: AIReport
):
    """Test that an admin can retrieve all AI reports."""
    response = client.get(
        "/internal/admin/ai-reports", headers={"Authorization": f"Bearer {admin_token}"}
    )
...
def test_update_ai_report_state(
    client: TestClient, session: Session, admin_token: str, sample_ai_report: AIReport
):
