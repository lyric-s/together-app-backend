...
def test_init_ai_test_data(session: Session):
    ...
    assert (
        "toxic" in toxic_user.volunteer_profile.first_name.lower()
        or "foutez le camp" in toxic_user.volunteer_profile.bio.lower()
    )

    # 3. Verify fraudulent mission created
    mission = session.exec(
        select(Mission).where(Mission.name == "ARGENT FACILE ET RAPIDE")
    ).first()
